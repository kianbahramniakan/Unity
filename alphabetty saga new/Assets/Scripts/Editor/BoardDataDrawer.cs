...
public class BoardDataDrawer : Editor
{
    ...
    {
        InitializeReordableList(ref _dataList, propertyName: "SearchWords", listLabel:"Searching Words");
    }
    ...
    private void ConvertToUpperButton()
    {
        if (GUILayout.Button(text:"To Upper"))
        {
            for (var i = 0; i < GameDataInstance.Columns; i++)
            {
                ...
                {
                    var errorCounter = Regex.Matches(input: GameDataInstance.Board[i].Row[j], pattern: @"[a-z]").Count;
    ...
    {
        if (GUILayout.Button(text:"Fill Up With Randow"))
        {
            for (int i = 0; i < GameDataInstance.Columns; i++)
            {
                for (int j = 0; j < GameDataInstance.Rows; j++)
